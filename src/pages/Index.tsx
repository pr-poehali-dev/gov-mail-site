...
export default function Index() {
  ...
  function handleScan() {
    ...
    const found = letters.find(
      (l) => l.trackingNumber === scanInput.trim().toUpperCase()
    );
    if (found) {
      setScanResult(found);
      setScanError(false);
  ...
  function handleTrack() {
    setTrackError(false);
    const found = letters.find(
      (l) => l.trackingNumber === trackInput.trim().toUpperCase()
    );
    if (found) {
  ...
  function handleUpdateStatus(id: string) {
    setLetters((prev) =>
      prev.map((l) => {
        ...
        const next =
          idx < STATUSES.length - 1 ? STATUSES[idx + 1] : l.status;
        return { ...l, status: next };
      })
    );
    if (trackResult?.id === id) {
      setTrackResult((prev) => {
        if (!prev) return null;
        ...
        return {
          ...prev,
          status:
            idx < STATUSES.length - 1 ? STATUSES[idx + 1] : prev.status,
        };
      });
    }
    if (scanResult?.id === id) {
    ...
    <div className="min-h-screen bg-[#f0eee8] font-golos">
          ...
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[var(--gov-gold)] flex items-center justify-center rounded-sm">
                <span className="text-[var(--gov-navy)] font-oswald font-bold text-lg">
                  ГП
                </span>
              </div>
              <div>
                <div className="font-oswald font-bold text-lg tracking-wider leading-tight">
                  ГОСПОЧТА
                </div>
                <div className="text-[var(--gov-gold)] text-xs tracking-widest font-golos">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {active === "home" && (
          <div className="animate-fade-in">
            <div className="bg-[var(--gov-navy)] text-white px-8 py-14 mb-8 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <h1 className="font-oswald text-4xl md:text-5xl font-bold tracking-wide mb-4 leading-tight">
                  ГОСУДАРСТВЕННАЯ
                  <br />
                  ПОЧТОВАЯ СЛУЖБА
                </h1>
                <p className="text-gray-300 font-golos text-lg mb-8 max-w-xl">
                  Надёжная отправка корреспонденции с полным отслеживанием и
                  цифровой регистрацией каждого письма.
                </p>
              ...
              {[
                {
                  ...
                  desc: "Заполните данные отправителя и получателя. Система присвоит уникальный номер отслеживания.",
                  action: "compose",
                },
                {
                  icon: "Printer",
                  title: "Печать конвертов",
                  desc: "Автоматическая генерация QR-кода и макета конверта для печати на стандартном принтере.",
                  action: "envelope",
                },
                {
                  icon: "QrCode",
                  title: "Сканирование QR",
                  desc: "Введите данные с QR-кода конверта для мгновенного получения информации об отправлении.",
                  action: "scan",
                },
                {
                  icon: "MapPin",
                  title: "Отслеживание",
                  desc: "Введите трек-номер и получите актуальный статус местонахождения вашего письма.",
                  action: "track",
                },
                {
                  icon: "ClipboardList",
                  title: "История отправлений",
                  desc: "Полный журнал всех зарегистрированных писем с возможностью просмотра и управления.",
                  action: "history",
                },
                {
                  icon: "ShieldCheck",
                  title: "Гарантия доставки",
                  desc: "Каждое письмо защищено уникальным идентификатором и регистрируется в государственной системе.",
                  action: "home",
                },
              ].map((s) => (
                <button
                  key={s.title}
                  onClick={() => navigate(s.action as Section)}
                  className="bg-white border border-gray-200 p-6 text-left hover:border-[var(--gov-navy)] hover:shadow-md transition-all group"
                >
                  <div className="w-10 h-10 bg-[var(--gov-navy)] flex items-center justify-center mb-4 group-hover:bg-[var(--gov-gold)] transition-colors">
                    <Icon
                      name={s.icon}
                      size={20}
                      className="text-[var(--gov-gold)] group-hover:text-[var(--gov-navy)]"
                    />
                  </div>
                  <h3 className="font-oswald font-bold text-[var(--gov-navy)] text-lg mb-2 tracking-wide">
                    {s.title}
                  </h3>
                  <p className="text-gray-500 text-sm font-golos leading-relaxed">
                    {s.desc}
                  </p>
                </button>
              ))}
            ...
            <div className="bg-[var(--gov-navy)] text-white p-6 grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="font-oswald text-3xl font-bold text-[var(--gov-gold)]">
                  {letters.length}
                </div>
                <div className="text-xs text-gray-400 font-golos tracking-wider mt-1">
                  ЗАРЕГИСТРИРОВАНО
                </div>
              </div>
              <div>
                <div className="font-oswald text-3xl font-bold text-[var(--gov-gold)]">
                  {letters.filter((l) => l.status === "Доставлено").length}
                </div>
                <div className="text-xs text-gray-400 font-golos tracking-wider mt-1">
                  ДОСТАВЛЕНО
                </div>
        ...
        {active === "compose" && (
          <div className="animate-fade-in">
            <SectionHeader
              icon="FileText"
              title="Оформление письма"
              subtitle="Заполните данные для регистрации отправления"
            />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 p-6">
                <div className="flex items-center gap-2 mb-5">
                  <div className="w-6 h-6 bg-[var(--gov-navy)] flex items-center justify-center">
                    <Icon
                      name="User"
                      size={13}
                      className="text-[var(--gov-gold)]"
                    />
                  </div>
                  <h3 className="font-oswald font-bold text-[var(--gov-navy)] tracking-wide">
                    ОТПРАВИТЕЛЬ
                  </h3>
                </div>
                <div className="space-y-4">
                  <Field
                    ...
                    onChange={(v) => handleFormChange("senderName", v)}
                    placeholder="Иванов Иван Иванович"
                  />
                  <Field
                    label="Адрес"
                    value={form.senderAddress}
                    onChange={(v) => handleFormChange("senderAddress", v)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      ...
                      onChange={(v) => handleFormChange("senderCity", v)}
                      placeholder="Москва"
                    />
                    <Field
                      label="Индекс"
                      value={form.senderZip}
                      onChange={(v) => handleFormChange("senderZip", v)}
                      placeholder="101000"
                    />
                  </div>
                </div>
              ...
              <div className="bg-white border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  <Field
                    ...
                    onChange={(v) => handleFormChange("recipientName", v)}
                    placeholder="Петров Пётр Петрович"
                  />
                  <Field
                    label="Адрес"
                    value={form.recipientAddress}
                    onChange={(v) => handleFormChange("recipientAddress", v)}
                    placeholder="пр. Победы, д. 22, кв. 8"
                  />
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      label="Город"
                      value={form.recipientCity}
                      onChange={(v) => handleFormChange("recipientCity", v)}
                      placeholder="Санкт-Петербург"
                    />
                    <Field
                      label="Индекс"
                      value={form.recipientZip}
                      onChange={(v) => handleFormChange("recipientZip", v)}
                      placeholder="190000"
                    />
                  </div>
                </div>
              ...
              <div className="bg-white border border-gray-200 p-6 md:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold font-oswald tracking-widest text-[var(--gov-navy)] uppercase mb-1">
                      Вид отправления
                    </label>
                    <select
                      value={form.type}
        ...
        {active === "envelope" && (
          <div className="animate-fade-in">
            <SectionHeader
              icon="Printer"
              title="Печать конверта"
              subtitle="Макет конверта с QR-кодом готов к печати"
            />
            ...
            {!currentLetter ? (
              <div className="bg-white border border-gray-200 p-12 text-center">
                ...
                />
                <p className="text-gray-500 font-golos mb-4">
                  Нет данных для печати. Сначала оформите письмо.
                </p>
                <button
                  onClick={() => navigate("compose")}
              ...
              <>
                ...
                <div
                  ref={printRef}
                  className="print-area bg-white p-8 max-w-3xl mx-auto shadow-sm"
                >
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-[var(--gov-navy)] flex items-center justify-center">
                        <span className="text-[var(--gov-gold)] font-oswald font-bold text-xs">
                          ГП
                        </span>
                      </div>
                      <div>
                        <div className="font-oswald font-bold text-[var(--gov-navy)] text-sm tracking-wider">
                          ГОСПОЧТА
                        </div>
                  ...
                  <div className="flex gap-8 mb-6">
                    <div className="flex-1">
                      <div className="text-xs text-gray-400 font-golos tracking-widest mb-2">
                        ОТПРАВИТЕЛЬ
                      </div>
                      <div className="font-golos text-sm font-semibold text-[var(--gov-navy)]">
                        {currentLetter.sender.name}
                      </div>
                      <div className="font-golos text-sm text-gray-600">
                        {currentLetter.sender.address}
                      </div>
                      <div className="font-golos text-sm text-gray-600">
                        {currentLetter.sender.city} {currentLetter.sender.zip}
                      </div>
                    </div>
                    <div className="flex-1 border-l-2 border-[var(--gov-gold)] pl-6">
                      ...
                      <div className="font-golos text-base font-bold text-[var(--gov-navy)]">
                        {currentLetter.recipient.name}
                      </div>
                      <div className="font-golos text-sm text-gray-600">
                        {currentLetter.recipient.address}
                      </div>
                      <div className="font-golos text-base font-semibold text-[var(--gov-navy)]">
                        {currentLetter.recipient.city}{" "}
                        {currentLetter.recipient.zip}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t-2 border-[var(--gov-navy)] pt-4 flex items-center justify-between bg-gray-50 px-4 py-3">
                    <div>
                      <div className="text-xs text-gray-400 font-golos tracking-widest mb-1">
                        ТРЕК-НОМЕР
                      </div>
                      <div className="font-oswald font-bold text-xl text-[var(--gov-navy)] tracking-widest">
                        {currentLetter.trackingNumber}
                      </div>
                    ...
                    <div className="flex flex-col items-center">
                      <QRCodeSVG
                        value={currentLetter.trackingNumber}
                        size={80}
                        bgColor="#ffffff"
                        fgColor="#0d1f4e"
                        level="H"
                      />
                      <div className="text-xs text-gray-400 mt-1 font-golos">
                        Сканируйте
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-4 p-4 bg-white border-l-4 border-[var(--gov-gold)] no-print">
                  <div className="flex items-center gap-2 text-sm font-golos text-green-700">
                    ...
                    Письмо зарегистрировано. Трек-номер:{" "}
                    <strong>{currentLetter.trackingNumber}</strong>
                  </div>
                </div>
              </>
            )}
        ...
        {active === "scan" && (
          <div className="animate-fade-in">
            <SectionHeader
              ...
              subtitle="Введите трек-номер с конверта для получения данных"
            />
            <div className="max-w-xl mx-auto">
              <div className="bg-white border border-gray-200 p-8">
                <div className="flex flex-col items-center mb-8">
                  ...
                  <p className="text-gray-500 text-sm font-golos text-center">
                    Введите трек-номер вручную или используйте сканер QR-кода
                  </p>
                </div>

                <div className="mb-4">
                  <label className="block text-xs font-semibold font-oswald tracking-widest text-[var(--gov-navy)] uppercase mb-1">
                    Трек-номер или данные QR-кода
                  </label>
                ...
                {scanError && (
                  <div className="p-3 bg-red-50 border-l-4 border-red-500 text-red-700 text-sm font-golos">
                    Отправление с таким номером не найдено в системе.
                  </div>
                )}
              ...
              {letters.length > 0 && (
                <div className="mt-4 bg-white border border-gray-200 p-5">
                  <div className="text-xs font-oswald tracking-wider text-gray-400 mb-3">
                    ПОСЛЕДНИЕ QR-КОДЫ
                  </div>
                    ...
                    {letters.slice(0, 4).map((l) => (
                      ...
                      >
                        <QRCodeSVG
                          ...
                          fgColor="#0d1f4e"
                        />
                        <div className="text-xs text-gray-400 mt-1 font-oswald">
                          {l.trackingNumber.slice(0, 10)}…
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {active === "track" && (
          <div className="animate-fade-in">
            <SectionHeader
              ...
              subtitle="Введите трек-номер для получения текущего статуса"
            />
            <div className="max-w-2xl mx-auto">
              <div className="bg-white border border-gray-200 p-6 mb-4">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={trackInput}
                    onChange={(e) => {
                      setTrackInput(e.target.value.toUpperCase());
                      setTrackResult(null);
                      setTrackError(false);
                    }}
              ...
              {trackError && (
                <div className="p-4 bg-red-50 border-l-4 border-red-500 text-red-700 text-sm font-golos">
                  Отправление с номером <strong>{trackInput}</strong> не
                  найдено. Проверьте номер и попробуйте снова.
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {letters.map((l) => (
                      <button
                        key={l.id}
                        onClick={() => {
                          setTrackInput(l.trackingNumber);
                          setTrackResult(l);
                        }}
                      ...
                      >
                        <div>
                          <div className="font-oswald font-bold text-[var(--gov-navy)] tracking-wide">
                            {l.trackingNumber}
                          </div>
                          <div className="text-xs text-gray-400 font-golos">
                            {l.recipient.name} · {l.recipient.city}
                          </div>
                        </div>
        ...
        {active === "history" && (
          <div className="animate-fade-in">
            <SectionHeader
              icon="ClipboardList"
              title="История отправлений"
              subtitle={`Зарегистрировано отправлений: ${letters.length}`}
            />
            ...
            {letters.length === 0 ? (
              <div className="bg-white border border-gray-200 p-12 text-center">
                ...
                <p className="text-gray-400 font-golos">
                  История пуста. Оформите первое письмо.
                </p>
                <button
                  onClick={() => navigate("compose")}
                  className="mt-4 bg-[var(--gov-navy)] text-[var(--gov-gold)] font-oswald px-6 py-2 tracking-wide hover:bg-[var(--gov-navy-light)]"
                >
                  ОФОРМИТЬ ПИСЬМО
                </button>
              </div>
              ...
              <div className="bg-white border border-gray-200 overflow-hidden overflow-x-auto">
                <table className="w-full text-sm font-golos min-w-[600px]">
                  <thead className="bg-[var(--gov-navy)] text-white">
                    <tr>
                      <th className="text-left px-4 py-3 font-oswald tracking-wider text-xs">
                        ТРЕК-НОМЕР
                      </th>
                      <th className="text-left px-4 py-3 font-oswald tracking-wider text-xs hidden md:table-cell">
                        ОТПРАВИТЕЛЬ
                      </th>
                      <th className="text-left px-4 py-3 font-oswald tracking-wider text-xs">
                        ПОЛУЧАТЕЛЬ
                    ...
                    {letters.map((l, i) => (
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="font-oswald font-bold text-[var(--gov-navy)] text-xs tracking-wide">
                            {l.trackingNumber}
                          </div>
      ...
      <footer className="no-print mt-12 bg-[var(--gov-navy)] text-white py-6 border-t-4 border-[var(--gov-gold)]">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-gray-400 font-golos">
          <div className="font-oswald text-[var(--gov-gold)] tracking-widest">
            ГОСПОЧТА
          </div>
          <div>
            © {new Date().getFullYear()} Государственная почтовая служба. Все
            права защищены.
          </div>
...
function Field({
  ...
  return (
    <div>
      <label className="block text-xs font-semibold font-oswald tracking-widest text-[#0d1f4e] uppercase mb-1">
        {label}
      </label>
      <input
...
function SectionHeader({
  ...
  return (
    <div className="mb-6 flex items-start gap-4">
      ...
      <div>
        <h2 className="font-oswald font-bold text-2xl text-[var(--gov-navy)] tracking-wide">
          {title}
        </h2>
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`text-xs px-2 py-1 border font-golos font-semibold whitespace-nowrap ${
  ...
  return (
    <div className="mt-4 border border-gray-200 bg-white">
      <div className="bg-[var(--gov-navy)] px-4 py-3 flex items-center justify-between">
        <div className="font-oswald font-bold text-white tracking-widest text-sm">
      ...
      <div className="p-4 grid grid-cols-2 gap-4 text-sm font-golos border-b border-gray-100">
        <div>
          <div className="text-xs text-gray-400 tracking-wider mb-1 font-oswald">
            ОТПРАВИТЕЛЬ
          </div>
          ...
          <div className="text-gray-500 text-xs">
            {letter.sender.city} {letter.sender.zip}
          </div>
        </div>
        <div>
          <div className="text-xs text-gray-400 tracking-wider mb-1 font-oswald">
          ...
          <div className="text-gray-500 text-xs">
            {letter.recipient.city} {letter.recipient.zip}
          </div>
        </div>
      </div>
      ...
      {showTimeline && (
        <div className="px-4 py-4 border-b border-gray-100">
          <div className="text-xs text-gray-400 tracking-wider mb-3 font-oswald">
            ИСТОРИЯ ДВИЖЕНИЯ
          </div>
          <div className="space-y-3">
            {STATUSES.map((s, i) => (
              <div
                key={s}
                className={`flex items-center gap-3 text-sm font-golos ${
                  i <= statusIdx ? "text-gray-800" : "text-gray-300"
                }`}
              >
                <div
                ...
                <span>{s}</span>
                {i === statusIdx && (
                  <span className="text-xs text-[var(--gov-gold)] font-semibold">
                    ← текущий
                  </span>
                )}
              </div>
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="text-xs text-gray-400 font-golos">
          {letter.type} · {letter.weight} · {letter.createdAt}
        </div>
        <button
